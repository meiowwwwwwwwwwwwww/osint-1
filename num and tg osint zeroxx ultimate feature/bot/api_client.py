...
class NumberApiResponseError(NumberApiError):
    """Raised when the number API response cannot be displayed safely."""
...
class NumberApiClient:
    ...
    async def lookup(self, number: str) -> NumberApiResponse:
        ...
        if not body.strip():
            logger.warning("Number API returned an empty response")
            raise NumberApiResponseError

        # Content-Type is the primary signal, but JSON-looking bodies are accepted too.
        looks_like_json = body.lstrip().startswith(("{", "[", '"')) or body.lstrip()[:1] in "-0123456789"
        if "json" in content_type or looks_like_json:
            try:
                return NumberApiResponse(content_type=content_type, payload=response.json(), is_json=True)
            except ValueError:
                if "json" in content_type:
                    logger.warning("Number API declared JSON but returned invalid JSON")
                    raise NumberApiResponseError
        ...
        if content_type and not (
            ...
            logger.warning("Number API returned an unsupported content type: %s", content_type)
            raise NumberApiResponseError

        # Plain text and HTML/text output are legitimate API results.  The handler
        # escapes it before sending it to Telegram.
...
class TelegramApiClient(NumberApiClient):
    ...
    async def lookup(self, telegram_user_id: str) -> NumberApiResponse:
        ...
        try:
            ...
            raise NumberApiHttpError from exc
        if not body.strip():
            logger.warning("Telegram API returned an empty response")
            raise NumberApiResponseError
        looks_like_json = body.lstrip().startswith(("{", "[", '"')) or body.lstrip()[:1] in "-0123456789"
        if "json" in content_type or looks_like_json:
            try:
